	...
	return {
		...
		initButton:function(){
	    	$('#activateuser').click(function (e) {
	    		if($('.checkboxes:checked').length>0){
		    		if(confirm($(this).attr('data-confirmmsg'))){
		    			$("#mainform").attr("action", domainurl+"user/activate");
		    			$('#mainform').submit();
		    		}
	    		}else{
	    	...
	    	$('#deactivateuser').click(function (e) {
	    		if($('.checkboxes:checked').length>0){
		    		if(confirm($(this).attr('data-confirmmsg'))){
		    			$("#mainform").attr("action", domainurl+"user/deactivate");
		    			$('#mainform').submit();
		    		}
				}else{
	    			App.growl($(this).attr('data-checkempty'),'warning');
	    	...
	    	$('#adduser').click(function (e) {
				window.location=domainurl+"user/add";
			});
	    	$('#removeuser').click(function (e) {
	    		if($('.checkboxes:checked').length>0){
		    		if(confirm($(this).attr('data-confirmmsg'))){
			    		$("#mainform").attr("action", domainurl+"/user/remove");
						$('#mainform').submit();
		    		}
	    		}else{
	    			App.growl($(this).attr('data-checkempty'),'warning');
        ...
        // begin first table
            ...
            /*"language": {
                ...
                "infoFiltered": "(filtered1 from _MAX_ "+table.attr('data-total')+" "+table.attr('data-entries')+")",
